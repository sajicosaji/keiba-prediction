...
# 2026-07-15 データ修復(列ズレ=通過順位リーク除去)後の正直な数字で再設定。
# 旧数字(205%等)はリークによる過大評価だった。信頼区間の下限は100%前後で、
# 「わずかにプラス期待」レベル。過信しないこと。
# 注: 実運用の発走前オッズは確定オッズより高めで、EVが水増しされる傾向。
# バックテスト(確定オッズ)ではEVを上げるほど回収率が下がる(EV1.8=125%,2.5=92%)ため
# 高EV=良ではない。頻度を抑えるためEV2.2(回収121%維持)+発走30分前オッズで運用する。
EV_MAIN_TH    = 2.2   # ◎: EV>=2.2 & オッズ<=50倍 → 修復後回収率 121% (1日約3.2R)
EV_MAIN_CAP   = 50.0
EV_LONG_TH    = 2.8   # ◎以外: EV>=2.8 & オッズ<=30倍・1レース1点 → 修復後回収率 99% (1日約0.9R)
EV_LONG_CAP   = 30.0  # 穴はオッズ上限を厳しく（旧EV3.0は修復後データで47%と判明）
EV_LONG_MAX   = 1     # 穴単勝は1レース最大1点（EV最大のみ）
BLEND_ALPHA  = 0.7   # モデル勝率の重み（市場勝率は 1-α）
DEFAULT_TEMP = 1.05  # calibration.json が無い場合のフォールバック温度（修復後較正値）
